...
const seedDatabase = async () => {
  try {
    ...
    const departments = await Department.insertMany([
      { name: 'Information Technology', code: 'IT', description: 'Manages digital infrastructure' },
      { name: 'Human Resources', code: 'HR', description: 'Manages workforce administration' },
      { name: 'Public Works', code: 'PWD', description: 'Infrastructure projects' },
      { name: 'Finance & Accounts', code: 'FIN', description: 'Budget and treasury' },
      { name: 'Health Services', code: 'HLT', description: 'Public health programs' },
    ]);
    ...
    const kpis = await KPI.insertMany([
      { title: 'Task Completion Rate', category: 'Task Completion', description: 'Monthly task delivery efficiency', department: itDept._id, targetValue: 100, unit: 'tasks', weightage: 30, evaluationCycle: 'Monthly', startDate, endDate, createdBy: admin._id, assignedTo: [employees[0]._id, employees[1]._id] },
      { title: 'Service Delivery Timeliness', category: 'Service Delivery', description: 'Average time to resolve requests', department: itDept._id, targetValue: 50, unit: 'requests', weightage: 25, evaluationCycle: 'Monthly', startDate, endDate, createdBy: admin._id, assignedTo: [employees[0]._id, employees[1]._id] },
      { title: 'Attendance Compliance', category: 'Attendance Compliance', description: 'Regularity and punctuality', department: pwdDept._id, targetValue: 26, unit: 'days', weightage: 20, evaluationCycle: 'Monthly', startDate, endDate, createdBy: admin._id, assignedTo: [employees[2]._id] },
      { title: 'Citizen Satisfaction Score', category: 'Citizen Satisfaction', description: 'Feedback score from citizens', department: hltDept._id, targetValue: 90, unit: 'percent', weightage: 25, evaluationCycle: 'Quarterly', startDate, endDate, createdBy: admin._id, assignedTo: [employees[5]._id] },
    ]);

    console.log('📝 Creating sample performance submissions...');
    await Performance.create([
      {
        employee: employees[0]._id, kpi: kpis[0]._id, taskDetails: 'Completed database migration project',
        achievedValue: 95, completionStatus: 'completed', status: 'approved', approvedScore: 95, finalScore: Math.round((95 * 30) / 100),
        supervisorRemarks: 'Excellent work', reviewedBy: supervisor1._id, reviewedAt: new Date(),
        evaluationPeriod: { month: now.getMonth() + 1, year: now.getFullYear() },
      },
      {
        employee: employees[1]._id, kpi: kpis[1]._id, taskDetails: 'Resolved 42 service requests',
